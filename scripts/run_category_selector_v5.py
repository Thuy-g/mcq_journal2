...
#
# Deterministic, auditable batch runner for category selector v3.5
# (src/category_extractor_ClaudeWeb_v5.py — the R1-safe hard class-leakage
# gate, Prompt 8H-B2-A).
#
# Derived from scripts/run_category_selector_v4.py at frozen hash
# b482028f52…, which remains unchanged (v4 is a frozen reference baseline).
# Only what v3.5 compatibility requires moved:
#
#   * the selector import (category_extractor_ClaudeWeb_v5, not _v4);
#   * the runner schema version;
#   * a NEW default SPARQL response-cache path, so a v5 run never writes into
#     v4's cache file even though the two cache SCHEMAS remain compatible
#     (query -> response is selector-version-independent; only the
#     leakage/selection decision differs downstream of the cache);
#   * one new summary.csv column (``selection_provenance``) and two new
#     ranking.csv columns (``leak_source``, ``leak_evidence``), reflecting the
#     v5 ``ClassSelectionResult``/``ClassCandidate`` fields;
#   * a resume-completeness rule that will not accept a v3.4 record as a
#     finished v3.5 Answer (the two schemas can disagree about whether the
#     same class was HARD-rejected, so a v3.4 result carried forward by
#     --resume would silently understate what v5 would have found).
#
...
from category_extractor_ClaudeWeb_v5 import (  # noqa: E402
    DEFAULT_ENDPOINT,
    DEFAULT_LANGUAGE,
    DEFAULT_SBERT_MODEL,
    DEFAULT_TOP_N,
...
# 1.2 (this file, v5): drives selector v3.5 (R1-safe hard class-leakage gate)
# instead of v3.4. summary.csv gained ``selection_provenance``; ranking.csv
# gained ``leak_source``/``leak_evidence``; --resume now also requires
# ``selection_provenance``, so a v3.4-selector (runner v1.1) result can never
# be carried forward as a finished v3.5 Answer — the two selector schemas can
# disagree about whether the same class was HARD-rejected (module docstring,
# ``v4_vs_v5_cached_regression.csv``), so silently reusing a v1.1 record would
# hide exactly the change this runner exists to measure.
RUNNER_SCHEMA_VERSION = "category_runner_v1.2"

# A NEW default cache path (§17: "a NEW v5 cache/output path, never
# overwriting v4 cache or batch-34 records"). The on-disk cache SCHEMA is
# unchanged and a v4 and a v5 run could technically share one cache file
# without corruption — a cached SPARQL response does not depend on which
# selector version later reads it — but this runner still defaults to a
# separate file so that no v5 invocation can ever mutate the cache file the
# frozen v4 batch-34 run and its provenance depend on.
DEFAULT_CACHE_PATH = os.environ.get(
    "CATEGORY_V5_SPARQL_CACHE", "./cache/category_v5_sparql.sqlite"
)
...
RESUMABLE_RESULT_FIELDS = frozenset(
    {
        ...
        # v3.5 provenance (v5 runner only). A record lacking this was written by
        # v3.4 or earlier, or by an interrupted write, and is not a finished
        # v3.5 Answer — see the runner-schema note on RUNNER_SCHEMA_VERSION.
...
def _summary_row(index: int, uri: str, record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "selection_provenance": _csv_scalar(record.get("selection_provenance")),
        ...
        rows.append(
            {
                ...
                "leak_source": _csv_scalar(candidate.get("leak_source")),
                "leak_evidence": ";".join(candidate.get("leak_evidence") or ()),
            }
        )
    return rows
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="run_category_selector_v5.py",
        description=(
            "Batch-run category selector v3.5 (R1-safe hard class-leakage "
            "gate) over a list of Answer "
