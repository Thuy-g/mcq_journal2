############################################################################
# tests/test_run_category_selector_v4.py
#
# Offline tests for scripts/run_category_selector_v4.py (selector v3.4).
#
...
# The v3.4 selector (category_extractor_ClaudeWeb_v4) is exercised for real —
# only its transport is replaced.
#
# Run:
#     python -m pytest -q tests/test_run_category_selector_v4.py
############################################################################
...
import run_category_selector_v4 as runner_mod  # noqa: E402
...
def test_manifest_records_the_frozen_selector_version_and_no_addresses(tmp_path):
    ...
    assert manifest["selector_schema_version"] == "category_selector_v3.4"
...
def test_importing_the_runner_creates_no_file_loads_no_model_and_opens_no_socket(tmp_path):
    ...
    probe.write_text(
        textwrap.dedent(
            ...
            import run_category_selector_v4 as mod
    ...
    assert report["selector_schema"] == "category_selector_v3.4"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A v3.3 record is complete for v3.3 but is not a v3.4 record.
        (
            "v3_3_record",
...
def test_the_frozen_selector_files_are_untouched():
    ...
    assert ce.SCHEMA_VERSION == "category_selector_v3.4", "the selector under test is v3.4"


# ---------------------------------------------------------------------------
# 14) v3.4 — redirect provenance reaches JSONL, summary.csv and the manifest
# ---------------------------------------------------------------------------
...
def test_manifest_reports_v34_selector_and_v11_runner(tmp_path):
    ...
    manifest = json.loads((tmp_path / "run_manifest.json").read_text(encoding="utf-8"))
    assert manifest["selector_schema_version"] == "category_selector_v3.4"
    assert manifest["runner_schema_version"] == "category_runner_v1.1"
    assert outcome.exit_code == 0
...
def test_resume_accepts_a_complete_v34_record(tmp_path):
    run_provenance_batch(tmp_path)
