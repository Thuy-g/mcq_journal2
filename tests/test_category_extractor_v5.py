############################################################################
# tests/test_category_extractor_v5.py
#
# Unit tests for src/category_extractor_ClaudeWeb_v5.py (selector v3.5).
#
# Derived from tests/test_category_extractor_v4.py at frozen hash
# 921ff6d407…, which remains unchanged (v4 is a frozen reference baseline;
# see CLAUDE.md's file-safety rule and the v5 module docstring). Every v4
# behavioural test not about class-name leakage is retained UNCHANGED, because
# v5's only scientific change relative to v4 is which rule decides HARD
# class-name leakage on the selection path (Prompt 8H-B2-A):
#
#   * SCHEMA_VERSION is now category_selector_v3.5;
#   * HARD/SOFT/NONE class-name leakage is decided by the frozen
#     ``rationale_v3.quality.detect_answer_leakage`` (R1) rule, run on the
#     canonical Answer URI and a resource-style adaptation of the category
#     URI — never by v4's label-based ``detect_leak()``, and never
#     overridable by nlp/synonym_expander;
#   * ``ClassCandidate`` gained ``leak_source``/``leak_evidence`` and
#     ``ClassSelectionResult`` gained ``selection_provenance``.
#
# CACHE_SCHEMA_VERSION is deliberately unchanged: no cache serialisation or
# key semantics moved, so existing caches stay valid, and v4/v5 may share one
# SQLite response cache (they never share an OUTPUT directory — see
# scripts/run_category_selector_v5.py).
#
...
# contacts an endpoint. The R1 leakage policy IS loaded from its real pinned
# file (``src/rationale_v3/policies/predicate_policy.json``) because that file
# is itself part of the frozen, offline, checked-in scientific configuration —
# loading it is not a network or model dependency.
#
...
#
# Run:
#     python -m pytest -q tests/test_category_extractor_v5.py -m "not integration"
############################################################################
...
import category_extractor_ClaudeWeb_v5 as ce  # noqa: E402
...
def test_import_creates_no_network_cache_or_model_side_effects(tmp_path):
    probe = textwrap.dedent(
        ...
        import category_extractor_ClaudeWeb_v5 as m
        ...
        assert m.SCHEMA_VERSION == "category_selector_v3.5", m.SCHEMA_VERSION
        print("PROBE_SCHEMA", m.SCHEMA_VERSION)
        print("PROBE_OK")
        """
    )
    ...
    assert "PROBE_SCHEMA category_selector_v3.5" in completed.stdout, (
        "the subprocess must have imported the v3.5 selector, not another version"
    )


def test_module_exposes_no_uppercase_best_and_report_has_no_optimality_wording():
    source = (SRC_DIR / "category_extractor_ClaudeWeb_v5.py").read_text(encoding="utf-8")
...
def test_soft_overlap_penalty_is_applied_only_when_configured():
    # v5: SOFT is now decided by R1 on URIs (Carbon / Boron_carbide, shared
    # prefix 'carb' = 4 chars, below the 5-char HARD threshold), so the Answer
    # URI itself — not just its client-reported label — must be the real
    # Carbon URI for the SOFT verdict to fire.
    answer_carbon = RESOURCE + "Carbon"
    target = CATEGORY + "Boron_carbide"
    client = make_client(
        label="Carbon",
        categories=(target, CAT_BIO),
        counts={target: 40, CAT_BIO: 800},
    )
    plain = ce.select_candidate_classes(answer_carbon, mode="recommended", client=client)
...
def test_v4_detect_leak_remains_importable_for_compatibility():
    """v4's label-based rule is still present (§8: 'keep it available'), but a
    test that exercises it here must not be mistaken for the v5 authoritative
    decision — see the tests below that show R1 overrules it."""
...
def test_hard_leak_is_detected_end_to_end_via_r1():
    """The full selection path, still HARD for a same-name-substring class —
    now decided by R1 on URIs, not by v4's label rule."""
    client = make_client(
        label="Shinya Yamanaka",
    ...
    rejected = next(
        c for c in result.evaluated_classes if c.category_uri == CATEGORY + "Shinya_Yamanaka_awards"
    )
    assert rejected.leak_source == ce.LEAKAGE_SOURCE_R1
    assert rejected.leak_reason == "ANSWER_LEXICAL_LEAK"
    assert rejected.leak_evidence  # matched-token evidence is recorded


def test_soft_overlap_is_flagged_but_never_hard_rejected():
    """Carbon / Boron_carbide: R1 SOFT only (shared prefix 'carb', 4 chars) —
    the audit's canonical 'must not be unjustifiably HARD' case."""
    answer_carbon = RESOURCE + "Carbon"
    cat_boron_carbide = CATEGORY + "Boron_carbide"
    client = make_client(
        label="Carbon",
        categories=(cat_boron_carbide,),
        counts={cat_boron_carbide: 40},
    )
    result = ce.select_candidate_classes(answer_carbon, mode="recommended", client=client)
    assert result.selection_status is ce.SelectionStatus.OK
    assert result.recommended_classes[0].leak_level is ce.LeakLevel.SOFT
    assert result.recommended_classes[0].leak_reason == "ANSWER_LEXICAL_LEAK_SOFT"
    assert result.number_rejected_by_reason == {}


def test_wordnet_and_synonym_expander_cannot_manufacture_a_hard_rejection():
    """A synonym expander/nlp pair that v4/v2 would treat as a leak signal must
    not affect the v5 selection outcome at all: R1 alone is authoritative."""
    ...
    result = ce.select_candidate_classes(
        ...
        client=client,
        nlp=FakeNlp(),
        synonym_expander=everything_is_related,
    )
    assert result.selection_status is ce.SelectionStatus.OK
    assert result.recommended_classes[0].leak_level is ce.LeakLevel.NONE
    assert result.config["nlp_supplied"] is True
    assert result.config["synonym_expander_supplied"] is True


def test_wordnet_style_expansion_alone_cannot_reject_a_class():
    """The pure v4 function, unchanged, kept importable: SOFT only, never HARD."""
...
# ---------------------------------------------------------------------------
# 19B) The frozen R1 gate — named regression cases (Prompt 8H-B2-A §11-12)
# ---------------------------------------------------------------------------

ANSWER_ALBERT_EINSTEIN = RESOURCE + "Albert_Einstein"
ANSWER_CARBON = RESOURCE + "Carbon"
...
R1_HARD_CASES = [
    pytest.param(
        ANSWER_ALBERT_EINSTEIN, CATEGORY + "Einstein_family", "WHOLE_TOKEN",
        id="albert_einstein-einstein_family",
    ),
    pytest.param(
        ANSWER_CARBON, CATEGORY + "Carbonado", "LONG_PREFIX",
        id="carbon-carbonado",
    ),
    pytest.param(
        ANSWER_ADAM_SMITH, CATEGORY + "Smithsonian_Institution", "LONG_PREFIX",
        id="adam_smith-smithsonian_institution",
    ),
    pytest.param(
        ANSWER_PLATO, CATEGORY + "Platonists", "LONG_PREFIX",
        id="plato-platonists",
    ),
]
...
R1_NOT_HARD_CASES = [
    ...
    pytest.param(
        ANSWER_ALBERT_EINSTEIN, CATEGORY + "Mileva_Mari%C4%87", id="einstein-mileva_maric"
    ),
...
def test_named_regression_cases_must_be_hard(answer_uri, category_uri, expected_kind):
    policy = ce.load_r1_leakage_policy()
    verdict = ce.classify_class_leakage(answer_uri, category_uri, policy=policy)
    assert verdict.level is ce.LeakLevel.HARD
    assert any(expected_kind in evidence for evidence in verdict.evidence), verdict.evidence


@pytest.mark.parametrize("answer_uri, category_uri", R1_NOT_HARD_CASES)
def test_named_regression_cases_must_not_be_unjustifiably_hard(answer_uri, category_uri):
    policy = ce.load_r1_leakage_policy()
    verdict = ce.classify_class_leakage(answer_uri, category_uri, policy=policy)
    assert verdict.level is not ce.LeakLevel.HARD


_NAMED_PAIRS_FOR_EQUIVALENCE = [
    (ANSWER_ALBERT_EINSTEIN, CATEGORY + "Einstein_family"),
    (ANSWER_CARBON, CATEGORY + "Carbonado"),
    (ANSWER_ADAM_SMITH, CATEGORY + "Smithsonian_Institution"),
    (ANSWER_PLATO, CATEGORY + "Platonists"),
    (ANSWER_CARBON, CATEGORY + "Boron_carbide"),
    (ANSWER_ALBERT_EINSTEIN, CATEGORY + "Mileva_Mari%C4%87"),
    (ANSWER_SILICON, CATEGORY + "Boron"),
]


@pytest.mark.parametrize("answer_uri, category_uri", _NAMED_PAIRS_FOR_EQUIVALENCE)
def test_v5_adapted_leakage_result_matches_the_direct_frozen_r1_function(
    answer_uri, category_uri
):
    """Proves v5 really reuses R1 rather than reimplementing an equivalent
    rule: the adapted-class verdict and a direct call to
    ``detect_answer_leakage`` over the same adapted URI must agree on
    hard/soft/none for every named regression pair (§12)."""
    policy = ce.load_r1_leakage_policy()
    adapted = ce.category_uri_to_resource_uri(category_uri)
    assert adapted is not None
    direct = detect_answer_leakage(answer_uri, adapted, policy)
    via_v5 = ce.classify_class_leakage(answer_uri, category_uri, policy=policy)
    if direct.hard_leak:
        expected_level = ce.LeakLevel.HARD
    elif direct.soft_leak:
        expected_level = ce.LeakLevel.SOFT
    else:
        expected_level = ce.LeakLevel.NONE
    assert via_v5.level is expected_level
    assert via_v5.reason == direct.reason_code


def test_display_label_preservation_iron_and_phosphorus():
    """§11 'Display-label preservation': the two iron chlorides must remain
    distinct rendered labels, independent of how they compare for leakage."""
    iron_ii = ce.format_display_label(RESOURCE + "Iron(II)_chloride")
    iron_iii = ce.format_display_label(RESOURCE + "Iron(III)_chloride")
    phosphorus_v_oxide = ce.format_display_label(RESOURCE + "Phosphorus(V)_oxide")
    assert iron_ii == "Iron(II) chloride"
    assert iron_iii == "Iron(III) chloride"
    assert iron_ii != iron_iii
    assert phosphorus_v_oxide == "Phosphorus(V) oxide"


# ---------------------------------------------------------------------------
# 19C) The category-URI -> resource-URI adapter
# ---------------------------------------------------------------------------


def test_category_uri_to_resource_uri_drops_only_the_category_marker():
    assert (
        ce.category_uri_to_resource_uri(CATEGORY + "Einstein_family")
        == RESOURCE + "Einstein_family"
    )


def test_category_uri_to_resource_uri_accepts_the_documented_spellings():
    assert (
        ce.category_uri_to_resource_uri("dbc:Einstein_family") == RESOURCE + "Einstein_family"
    )
    assert (
        ce.category_uri_to_resource_uri("Category:Einstein_family")
        == RESOURCE + "Einstein_family"
...
def test_category_uri_to_resource_uri_preserves_percent_encoding():
    adapted = ce.category_uri_to_resource_uri(CATEGORY + "Mileva_Mari%C4%87")
    assert adapted == RESOURCE + "Mileva_Mari%C4%87"


def test_category_uri_to_resource_uri_preserves_parentheses_and_colons():
    """The adapter must not perform display normalisation or stemming: it is a
    LEXICAL rewrite only (drop 'Category:'), never a display transform."""
    assert (
        ce.category_uri_to_resource_uri(CATEGORY + "Phosphorus(V)_compounds")
        == RESOURCE + "Phosphorus(V)_compounds"
    )
    assert (
        ce.category_uri_to_resource_uri(CATEGORY + "Star_Trek:_Voyager_episodes")
        == RESOURCE + "Star_Trek:_Voyager_episodes"
    )


def test_category_uri_to_resource_uri_rejects_foreign_or_malformed_input():
    assert ce.category_uri_to_resource_uri("https://evil.example/resource/Category:X") is None
    assert ce.category_uri_to_resource_uri("http://dbpedia.org/resource/FooCategory:Bar") is None
    assert ce.category_uri_to_resource_uri(None) is None
    assert ce.category_uri_to_resource_uri("") is None


def test_classify_class_leakage_is_total_for_an_unadaptable_category():
    """A category that fails adaptation reports NONE rather than raising —
    defensive, since every caller inside this module already validates the
    category URI earlier in the pipeline."""
    policy = ce.load_r1_leakage_policy()
    verdict = ce.classify_class_leakage(
        ANSWER_ALBERT_EINSTEIN, "https://evil.example/resource/Category:X", policy=policy
    )
    assert verdict.level is ce.LeakLevel.NONE


# ---------------------------------------------------------------------------
# 19D) Albert Einstein fixture regression (§13)
# ---------------------------------------------------------------------------

CAT_EINSTEIN_FAMILY = CATEGORY + "Einstein_family"
CAT_GERMAN_THEORETICAL_PHYSICISTS = CATEGORY + "German_theoretical_physicists"
CAT_NOBEL_LAUREATES_IN_PHYSICS = CATEGORY + "Nobel_laureates_in_Physics"
CAT_20TH_CENTURY_GERMAN_PHYSICISTS = CATEGORY + "20th-century_German_physicists"


def _einstein_client(*, member=False):
    counts = {
        CAT_EINSTEIN_FAMILY: 12,
        CAT_GERMAN_THEORETICAL_PHYSICISTS: 73,
        CAT_NOBEL_LAUREATES_IN_PHYSICS: 218,
        CAT_20TH_CENTURY_GERMAN_PHYSICISTS: 561,
    }
...
def test_einstein_family_is_rejected_as_hard_leakage():
    result = ce.select_candidate_classes(
        ANSWER_ALBERT_EINSTEIN, mode="recommended", client=_einstein_client()
    )
    rejected = next(
        c for c in result.evaluated_classes if c.category_uri == CAT_EINSTEIN_FAMILY
    )
    assert rejected.feasible is False
    assert rejected.leak_level is ce.LeakLevel.HARD
    assert rejected.rejected_code == ce.REASON_HARD_LEAK
    assert CAT_EINSTEIN_FAMILY not in {c.category_uri for c in result.recommended_classes}


def test_einstein_family_can_never_become_selected_across_all_modes():
    for mode in ("recommended", "first_feasible"):
        result = ce.select_candidate_classes(
            ANSWER_ALBERT_EINSTEIN, mode=mode, client=_einstein_client()
        )
        assert result.selected_class != CAT_EINSTEIN_FAMILY

    # member=True: the Answer *is* a confirmed member of Einstein_family, so
    # this exercises the leakage rejection specifically, not the (also
    # sufficient) not-a-member rejection.
    manual_result = ce.select_candidate_classes(
        ANSWER_ALBERT_EINSTEIN,
        mode="manual",
        requested_class=CAT_EINSTEIN_FAMILY,
        client=_einstein_client(member=True),
    )
    assert manual_result.selection_status is ce.SelectionStatus.MANUAL_CLASS_INVALID
    assert manual_result.selected_class is None
    assert manual_result.evaluated_classes[0].rejected_code == ce.REASON_HARD_LEAK


def test_automatic_recommended_mode_proceeds_to_surviving_classes_not_a_hardcoded_exception():
    """The selected class must come from the unchanged ranking/feasibility
    logic over the surviving classes, not from any Einstein-specific code."""
    result = ce.select_candidate_classes(
        ANSWER_ALBERT_EINSTEIN, mode="recommended", client=_einstein_client()
    )
    assert result.selection_status is ce.SelectionStatus.OK
    assert result.selected_class in {
        CAT_GERMAN_THEORETICAL_PHYSICISTS,
        CAT_NOBEL_LAUREATES_IN_PHYSICS,
        CAT_20TH_CENTURY_GERMAN_PHYSICISTS,
    }
    assert result.selection_provenance == ce.SELECTION_PROVENANCE_AUTOMATIC
    # The three surviving classes are ranked by the unmodified IDF-only
    # specificity policy (no encoder supplied): the most specific (smallest
    # remote_count) class ranks first, exactly as in v4.
    assert result.selected_class == CAT_GERMAN_THEORETICAL_PHYSICISTS


def test_source_of_v5_module_never_special_cases_the_einstein_string():
    """§13: 'Do NOT special-case the string Albert_Einstein in selection
    code.' Verified structurally: the literal never appears outside comments/
    docstrings/the offline self-check demo at the bottom of the file."""
    source = (SRC_DIR / "category_extractor_ClaudeWeb_v5.py").read_text(encoding="utf-8")
    lines = source.splitlines()
    demo_start = next(i for i, line in enumerate(lines) if line.startswith('if __name__'))
    for index, line in enumerate(lines):
        stripped = line.strip()
        if "Albert_Einstein" not in line and "Albert Einstein" not in line:
            continue
        is_comment_or_doc = stripped.startswith("#") or '"""' in line or "'''" in line or (
            index > demo_start
        )
        assert is_comment_or_doc, f"non-comment Einstein reference at line {index + 1}: {line!r}"


# ---------------------------------------------------------------------------
# 19E) v5 provenance: selection_provenance, leak_source, policy hash
# ---------------------------------------------------------------------------


def test_automatic_selection_is_stamped_with_the_explicit_provenance_token():
    result = ce.select_candidate_classes(ANSWER, mode="recommended", client=make_client())
    assert result.selection_status is ce.SelectionStatus.OK
    assert result.selection_provenance == ce.SELECTION_PROVENANCE_AUTOMATIC
    assert result.selection_provenance != "HUMAN_APPROVED"


def test_manual_selection_is_stamped_with_a_distinct_non_automatic_token():
    client = make_client(member=True)
    result = ce.select_candidate_classes(
        ANSWER, mode="manual", requested_class=CAT_NOBEL, client=client
    )
    assert result.selection_status is ce.SelectionStatus.OK
    assert result.selection_provenance == ce.SELECTION_PROVENANCE_MANUAL
    assert result.selection_provenance != ce.SELECTION_PROVENANCE_AUTOMATIC
    assert result.selection_provenance != "HUMAN_APPROVED"


def test_no_selection_provenance_when_no_class_was_selected():
    client = make_client(categories=())
    result = ce.select_candidate_classes(ANSWER, mode="recommended", client=client)
    assert result.selected_class is None
    assert result.selection_provenance is None


def test_leakage_policy_provenance_is_recorded_in_config():
    result = ce.select_candidate_classes(ANSWER, mode="recommended", client=make_client())
    assert result.config["leakage_implementation"] == ce.LEAKAGE_SOURCE_R1
    policy = ce.load_r1_leakage_policy()
    assert result.config["leakage_policy_sha256"] == policy.policy_sha256
    assert result.config["leakage_policy_version"] == policy.version


def test_load_r1_leakage_policy_is_cached_and_matches_the_pinned_file():
    policy_a = ce.load_r1_leakage_policy()
    policy_b = ce.load_r1_leakage_policy()
    assert policy_a is policy_b  # cached, not re-read every call
    assert policy_a.policy_sha256 == hashlib.sha256(
        ce.DEFAULT_QUALITY_POLICY_PATH.read_bytes()
    ).hexdigest()
...
def test_result_schema_version_is_bumped_and_the_cache_schema_is_not():
    ...
    # 3.4: redirect provenance is serialised, and an apostrophe is accepted in
    # an IRIREF, so 3.3 records may be missing Answers that 3.4 resolves.
    # 3.5 (v5): the authoritative HARD class-name leakage decision moved to R1,
    # so a 3.4 and a 3.5 record may disagree on whether the same class was
    # HARD-rejected — see the v5 module docstring.
    assert ce.SCHEMA_VERSION == "category_selector_v3.5"
...
def test_serialised_results_carry_the_new_schema_version():
    ...
    assert result.schema_version == "category_selector_v3.5"
    assert result.to_dict()["schema_version"] == "category_selector_v3.5"
    assert json.loads(result.to_json())["schema_version"] == "category_selector_v3.5"
...
@pytest.mark.parametrize(
    "value,expected",
    [
        # v3.4: an apostrophe is legal inside <...>, so it no longer disqualifies
        # a category. It was in the v3.3 reject list purely because one character
        # class guarded both the IRIREF and the quoted-literal contexts.
        ("dbc:X'", CATEGORY + "X'"),
...
def test_query_builders_refuse_unvalidated_input():
    ...
    with pytest.raises(ValueError):  # v3.4: backslash could start an escape
        ce.build_class_members_query(CAT_NOBEL, after="x\\y")
    with pytest.raises(ValueError):
...
# ---------------------------------------------------------------------------
# 14) v3.4 — validation split by syntactic context
# ---------------------------------------------------------------------------

RAW_APOSTROPHE_IRI = RESOURCE + "Shin'ichirō_Tomonaga"
# SPARQL 1.1 rule [139]: '<' ([^<>"{}|^`\] - [#x00-#x20])* '>'
IRIREF_BODY = re.compile(r"^[^<>\"{}|^`\\\x00-\x20]*$")


def test_apostrophe_iri_is_accepted_and_never_percent_encoded():
    """The defect this version exists to fix."""
    assert IRIREF_BODY.match(RAW_APOSTROPHE_IRI), "premise: the IRI is a legal IRIREF"
    assert ce.is_safe_uri(RAW_APOSTROPHE_IRI) is True

    for build in (
        ce.build_answer_info_query,
        ce.build_answer_categories_query,
        ce.build_redirect_query,
    ):
        query = build(RAW_APOSTROPHE_IRI)
        assert f"<{RAW_APOSTROPHE_IRI}>" in query, build.__name__
        assert "%27" not in query and "%C5%8D" not in query, (
            f"{build.__name__} percent-encoded the IRI; %27 is a different RDF term"
        )
...
@pytest.mark.parametrize(
    "forbidden, uri",
    [
        ("space", "http://dbpedia.org/resource/A B"),
        ("less-than", "http://dbpedia.org/resource/A<B"),
        ("greater-than", "http://dbpedia.org/resource/A>B"),
        ("double-quote", 'http://dbpedia.org/resource/A"B'),
        ("open-brace", "http://dbpedia.org/resource/A{B"),
        ("close-brace", "http://dbpedia.org/resource/A}B"),
        ("pipe", "http://dbpedia.org/resource/A|B"),
        ("caret", "http://dbpedia.org/resource/A^B"),
        ("backtick", "http://dbpedia.org/resource/A`B"),
        ("backslash", "http://dbpedia.org/resource/A\\B"),
        ("newline", "http://dbpedia.org/resource/A\nB"),
        ("tab", "http://dbpedia.org/resource/A\tB"),
        ("nul", "http://dbpedia.org/resource/A\x00B"),
        ("c1-control", "http://dbpedia.org/resource/A\x85B"),
        ("bad-scheme", "ftp://dbpedia.org/resource/A"),
        ("no-scheme", "dbpedia.org/resource/A"),
    ],
)
def test_iriref_validation_is_not_weakened_for_anything_else(forbidden, uri):
    assert ce.is_safe_uri(uri) is False, forbidden
    with pytest.raises(ValueError):
        ce.build_answer_info_query(uri)


@pytest.mark.parametrize(
    "value, accepted",
    [
        ("http://dbpedia.org/resource/O'Reilly_Media", True),
        ("plain-string", True),
        ('has"quote', False),
        ("has\\backslash", False),
        ("has\nnewline", False),
...
def test_quoted_literal_context_is_validated_independently(value, accepted):
    """CONTEXT B is its own predicate, not the IRIREF one."""
    assert ce.is_safe_literal(value) is accepted


def test_the_two_contexts_disagree_where_they_should():
    """A plain string is fine in a literal and not an IRI; both reject a quote."""
    assert ce.is_safe_literal("plain-string") is True
    assert ce.is_safe_uri("plain-string") is False
    assert ce.is_safe_literal('a"b') is False
    assert ce.is_safe_uri('http://dbpedia.org/resource/a"b') is False
    # And the apostrophe is accepted by both.
    assert ce.is_safe_literal("a'b") is True
    assert ce.is_safe_uri(RAW_APOSTROPHE_IRI) is True


def test_pagination_cursor_accepts_an_apostrophe_bearing_member():
    cursor = RESOURCE + "O'Reilly_Media"
    query = ce.build_class_members_query(CAT_NOBEL, limit=10, after=cursor)
    assert f'FILTER(STR(?x) > "{cursor}")' in query
    assert "%27" not in query


# ---------------------------------------------------------------------------
# 15) v3.4 — redirect resolution state machine
# ---------------------------------------------------------------------------
...
class _StubResolver:
    ...
    def resolve(self, uri):
        ...
        if callable(self.outcome):
            return self.outcome(uri)
        return self.outcome


def test_redirect_status_not_attempted_when_disabled():
    r = ce.resolve_query_uri_with_provenance(ANSWER, resolver=_StubResolver(None))
    assert r.status is ce.RedirectStatus.NOT_ATTEMPTED
    assert r.redirect_attempted is False
    assert r.redirect_target is None
    assert r.redirect_used is False
    assert r.rejected_reason is None
    assert r.query_uri == ANSWER


def test_redirect_status_no_redirect_on_a_successful_zero_result():
    stub = _StubResolver(None)
    r = ce.resolve_query_uri_with_provenance(ANSWER, resolver=stub, enabled=True)
    assert r.status is ce.RedirectStatus.NO_REDIRECT
    assert r.redirect_attempted is True
    assert r.redirect_target is None
    assert r.redirect_used is False
    assert r.rejected_reason is None
    assert stub.calls == [ANSWER]


def test_redirect_status_followed_preserves_the_exact_raw_target():
    target = RAW_APOSTROPHE_IRI
    r = ce.resolve_query_uri_with_provenance(
        ANSWER, resolver=_StubResolver(target), enabled=True
    )
    assert r.status is ce.RedirectStatus.FOLLOWED
    assert r.redirect_attempted is True
    assert r.redirect_target == target
    assert r.query_uri == target
    assert r.redirect_used is True
    assert r.rejected_reason is None
    assert "%27" not in r.query_uri


def test_redirect_status_rejected_for_an_unsafe_target():
    bad = RESOURCE + "Invalid Target"
    r = ce.resolve_query_uri_with_provenance(
        ANSWER, resolver=_StubResolver(bad), enabled=True
    )
    assert r.status is ce.RedirectStatus.REJECTED
    assert r.redirect_attempted is True
    assert r.redirect_target == bad, "the exact returned string is preserved"
    assert r.query_uri == ANSWER
    assert r.redirect_used is False
    assert r.rejected_reason == ce.REDIRECT_REASON_UNSAFE_TARGET


def test_redirect_status_rejected_for_a_cycle_with_a_machine_readable_reason():
    r = ce.resolve_query_uri_with_provenance(
        ANSWER, resolver=_StubResolver(ANSWER), enabled=True, max_hops=3
    )
    assert r.status is ce.RedirectStatus.REJECTED
    assert r.rejected_reason == ce.REDIRECT_REASON_CYCLE
    assert r.redirect_target == ANSWER
    assert r.redirect_used is False
    assert r.query_uri == ANSWER


def test_redirect_status_query_failed_is_distinct_from_no_redirect():
    r = ce.resolve_query_uri_with_provenance(
        ANSWER,
        resolver=_StubResolver(ce.RedirectQueryFailed(ANSWER, "endpoint refused")),
        enabled=True,
    )
    assert r.status is ce.RedirectStatus.QUERY_FAILED
    assert r.status is not ce.RedirectStatus.NO_REDIRECT
    assert r.redirect_attempted is True
    assert r.redirect_used is False
    assert r.error == "endpoint refused"


def test_redirect_status_resolver_error_is_contained_but_visible():
    r = ce.resolve_query_uri_with_provenance(
        ANSWER, resolver=_StubResolver(ValueError("resolver bug")), enabled=True
    )
    assert r.status is ce.RedirectStatus.RESOLVER_ERROR
    assert r.status is not ce.RedirectStatus.NO_REDIRECT
    assert r.redirect_attempted is True
    assert r.redirect_used is False
    assert "resolver bug" in (r.error or "")


def test_compatibility_wrapper_keeps_the_v3_tuple_api():
    assert ce.resolve_query_uri(ANSWER, resolver=_StubResolver(None)) == (ANSWER, False)
    target = RESOURCE + "Target"
    assert ce.resolve_query_uri(
        ANSWER, resolver=_StubResolver(target), enabled=True
    ) == (target, True)
    with pytest.raises(ce.RedirectQueryFailed):
        ce.resolve_query_uri(
            ANSWER,
            resolver=_StubResolver(ce.RedirectQueryFailed(ANSWER, "boom")),
            enabled=True,
        )


def test_resolution_serialises_the_four_provenance_fields():
    r = ce.resolve_query_uri_with_provenance(
        ANSWER, resolver=_StubResolver(RESOURCE + "Bad Target"), enabled=True
    )
    assert r.to_dict() == {
        "redirect_attempted": True,
        "redirect_target": RESOURCE + "Bad Target",
        "redirect_resolution_status": "rejected",
        "redirect_rejected_reason": ce.REDIRECT_REASON_UNSAFE_TARGET,
    }


# ---------------------------------------------------------------------------
# 16) v3.4 — provenance reaches every serialized result
# ---------------------------------------------------------------------------
...
def _record(**kwargs):
    return ce.select_candidate_classes(**kwargs).to_dict()


def test_every_result_serialises_provenance_even_on_early_returns():
    client = make_client()
    cases = {
        "ok": dict(answer_uri=ANSWER, mode="recommended", client=client),
        "no_categories": dict(
            answer_uri=ANSWER, mode="recommended", client=make_client(categories=())
        ),
        "query_failed": dict(
            answer_uri=ANSWER,
            mode="recommended",
        ...
        "local_uri_not_found": dict(
            answer_uri=ANSWER,
            mode="recommended",
            client=make_client(),
            local_index={"<http://dbpedia.org/resource/Other>": 0},
        ),
    }
    for name, kwargs in cases.items():
        record = _record(**kwargs)
        for field in PROVENANCE_FIELDS:
            assert field in record, f"{name}: missing {field}"
        assert record["redirect_resolution_status"] in {s.value for s in ce.RedirectStatus}


def test_an_ordinary_selection_reports_not_attempted():
    record = _record(answer_uri=ANSWER, mode="recommended", client=make_client())
    assert record["redirect_attempted"] is False
    assert record["redirect_target"] is None
    assert record["redirect_resolution_status"] == "not_attempted"
    assert record["redirect_rejected_reason"] is None
    assert record["redirect_used"] is False


def test_a_zero_result_lookup_reports_no_redirect():
    client = make_client(categories=(), label=None, redirect_map={})
    record = _record(
        answer_uri=ANSWER,
        mode="recommended",
        client=client,
        resolve_redirects=True,
        redirect_resolver=ce.SparqlRedirectResolver(ce.SparqlRunner(client=client)),
    )
    assert record["redirect_attempted"] is True
    assert record["redirect_target"] is None
    assert record["redirect_resolution_status"] == "no_redirect"
    assert record["redirect_used"] is False


def test_a_followed_redirect_reports_the_target_and_ok():
    target = RESOURCE + "Canonical_Target"

    class TwoStage(FakeSparqlClient):
        def _rows(self, qid, query):
            if qid == "answer_categories" and f"<{ANSWER}>" in query:
                return []
            return super()._rows(qid, query)
    ...
    result = ce.select_candidate_classes(
        ...
        runner=runner,
        resolve_redirects=True,
        redirect_resolver=ce.SparqlRedirectResolver(runner),
    )
    record = result.to_dict()
    assert record["original_uri"] == ANSWER, "never rewritten"
    assert record["query_uri"] == target
    assert record["redirect_used"] is True
    assert record["redirect_attempted"] is True
    assert record["redirect_target"] == target
    assert record["redirect_resolution_status"] == "followed"
    assert record["redirect_rejected_reason"] is None
    assert record["selection_status"] == "ok"


def test_a_rejected_target_is_reported_and_never_queried():
    bad = RESOURCE + "Bad Target Value"

    class BadRedirect(FakeSparqlClient):
        def _rows(self, qid, query):
            if qid == "answer_categories" and f"<{ANSWER}>" in query:
                return []
            if qid == "redirect":
                return [{"target": self._cell(bad)}]
            return super()._rows(qid, query)

    client = BadRedirect(label=None, categories=(), counts={})
    runner = ce.SparqlRunner(client=client)
    record = ce.select_candidate_classes(
        ANSWER,
        mode="recommended",
        runner=runner,
        resolve_redirects=True,
    ...
    assert record["redirect_attempted"] is True
    assert record["redirect_target"] == bad
    assert record["redirect_resolution_status"] == "rejected"
    assert record["redirect_rejected_reason"] == ce.REDIRECT_REASON_UNSAFE_TARGET
    assert record["redirect_used"] is False
    assert record["query_uri"] == ANSWER
    assert not [q for q in client.queries if f"<{bad}>" in q], "rejected target queried"


def test_a_failed_redirect_query_keeps_its_query_failed_semantics():
    client = make_client(categories=(), label=None, fail_qids={"redirect"})
    runner = ce.SparqlRunner(client=client)
    result = ce.select_candidate_classes(
        ANSWER,
        mode="recommended",
    ...
    assert result.query_status is ce.QueryStatus.FAILED
    assert record["redirect_attempted"] is True
    assert record["redirect_resolution_status"] == "query_failed"
    assert record["redirect_resolution_status"] != "no_redirect"
    assert record["redirect_used"] is False
    assert record["error"]


def test_shared_runner_counters_still_hold_with_provenance():
    """The v3.3 counter invariants are untouched by the new fields."""
    client = make_client()
    runner = ce.SparqlRunner(client=client)
    first = ce.select_candidate_classes(ANSWER, mode="recommended", runner=runner)
    second = ce.select_candidate_classes(
        RESOURCE + "Kenichi_Fukui", mode="recommended", runner=runner
    )
    assert first.sparql_query_count == first.cache_stats["queries"]
    assert second.sparql_query_count == second.cache_stats["queries"]
    assert runner.stats()["queries"] == (
        first.sparql_query_count + second.sparql_query_count
    )
...
# ---------------------------------------------------------------------------
# 17) Post-review corrections — cursor URI semantics and resolver-error detail
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "dbpedia.org/resource/X",
        'http://dbpedia.org/resource/A"B',
        "http://dbpedia.org/resource/A\\B",
        "http://dbpedia.org/resource/A\nB",
        "http://dbpedia.org/resource/A\x00B",
        "",
        None,
    ],
)
def test_pagination_cursor_still_requires_a_valid_http_iri(cursor):
    """The cursor is a member IRI, so v3.3's URI requirement is retained.

    The v3.4 context split must not widen this: a cursor that is not an http(s)
    IRI was rejected by v3.3 and is rejected here too.
    """
    if cursor:
        with pytest.raises(ValueError):
            ce.build_class_members_query(CAT_NOBEL, after=cursor)
    else:
        # A falsy cursor means "no keyset filter", not an error.
        assert 'FILTER(STR(?x) >' not in ce.build_class_members_query(
            CAT_NOBEL, after=cursor
        )


def test_pagination_cursor_accepts_an_apostrophe_iri_verbatim():
    cursor = RESOURCE + "O'Reilly_Media"
    query = ce.build_class_members_query(CAT_NOBEL, limit=10, after=cursor)
    assert f'FILTER(STR(?x) > "{cursor}")' in query
    assert "%27" not in query
...
def test_the_cursor_validator_composes_both_contexts_without_merging_them():
    """Both predicates are applied; neither character class absorbed the other."""
    cursor = RESOURCE + "O'Reilly_Media"
    assert ce.is_safe_uri(cursor) is True
    assert ce.is_safe_literal(cursor) is True
    assert ce._validate_cursor(cursor) == cursor
    # Fails CONTEXT A only (not an IRI) — still rejected.
    assert ce.is_safe_literal("plain-string") is True
    assert ce.is_safe_uri("plain-string") is False
    with pytest.raises(ValueError):
        ce._validate_cursor("plain-string")


def test_a_contained_resolver_error_keeps_its_detail_in_the_result():
    """A custom resolver's own bug is contained, reported, and not lost."""
    ...
    # Contained: an ordinary data outcome, never converted into query_failed.
    assert result.selection_status is not ce.SelectionStatus.QUERY_FAILED
    assert result.query_status is not ce.QueryStatus.FAILED
    assert record["selection_status"] == "no_label"

    assert record["redirect_resolution_status"] == "resolver_error"
    assert record["redirect_attempted"] is True
    assert record["redirect_used"] is False
    assert "resolver bug" in (record["error"] or ""), "resolver detail was lost"


def test_endpoint_error_still_wins_over_resolver_error_in_the_error_field():
    """Precedence: explicit error, then AnswerInfo error, then resolver error."""
    client = make_client(fail_qids={"answer_info"})

    class ExplodingResolver:
        def resolve(self, uri):  # pragma: no cover - never reached
            raise ValueError("resolver bug")
    ...
    assert record["selection_status"] == "query_failed"
    assert "simulated endpoint failure" in (record["error"] or "")
    assert "resolver bug" not in (record["error"] or "")
