############################################################################
# tests/test_category_extractor_v4.py
#
# Unit tests for src/category_extractor_ClaudeWeb_v4.py (selector v3.4).
#
# Derived from tests/test_category_extractor_v3.py at frozen hash
# 7f5b0461…, which remains unchanged. Every v3 behavioural test is retained.
# Only expectations v3.4 deliberately changed were updated:
#
#   * SCHEMA_VERSION is now category_selector_v3.4;
#   * an apostrophe is accepted in an IRIREF position and in a quoted literal;
#   * redirect resolution reports an explicit status and provenance.
#
# CACHE_SCHEMA_VERSION is deliberately unchanged: no cache serialisation or
# key semantics moved, so existing caches stay valid.
#
...
#
# Run:
#     python -m pytest -q tests/test_category_extractor_v4.py -m "not integration"
############################################################################
...
def test_import_creates_no_network_cache_or_model_side_effects(tmp_path):
    probe = textwrap.dedent(
        ...
        import category_extractor_ClaudeWeb_v4 as m
        ...
        assert m.SCHEMA_VERSION == "category_selector_v3.4", m.SCHEMA_VERSION
    ...
    assert "PROBE_SCHEMA category_selector_v3.4" in completed.stdout, (
        "the subprocess must have imported the v3.4 selector, not another version"
    )


def test_module_exposes_no_uppercase_best_and_report_has_no_optimality_wording():
    source = (SRC_DIR / "category_extractor_ClaudeWeb_v4.py").read_text(encoding="utf-8")
...
def test_result_schema_version_is_bumped_and_the_cache_schema_is_not():
    ...
    assert ce.SCHEMA_VERSION == "category_selector_v3.4"
...
def test_serialised_results_carry_the_new_schema_version():
    ...
    assert result.schema_version == "category_selector_v3.4"
    assert result.to_dict()["schema_version"] == "category_selector_v3.4"
    assert json.loads(result.to_json())["schema_version"] == "category_selector_v3.4"
