############################################################################
# tests/test_run_category_selector_v5.py
#
# Offline tests for scripts/run_category_selector_v5.py (selector v3.5).
#
# Derived from tests/test_run_category_selector_v3.py at frozen hash 142acee9…,
# which remains unchanged. Every applicable v3 runner test is retained; only
# schema strings and the resume-completeness fixture moved, plus new coverage
# for redirect provenance in JSONL, summary.csv and the manifest.
#
...
# The v3.5 selector (category_extractor_ClaudeWeb_v5) is exercised for real —
# only its transport is replaced.
#
# Run:
#     python -m pytest -q tests/test_run_category_selector_v5.py
############################################################################
...
import run_category_selector_v5 as runner_mod  # noqa: E402
...
def test_manifest_records_the_frozen_selector_version_and_no_addresses(tmp_path):
    ...
    assert manifest["selector_schema_version"] == "category_selector_v3.5"
...
def test_importing_the_runner_creates_no_file_loads_no_model_and_opens_no_socket(tmp_path):
    ...
    probe.write_text(
        textwrap.dedent(
            ...
            import run_category_selector_v5 as mod
    ...
    assert report["selector_schema"] == "category_selector_v3.5"
...
def _complete_normal_record(uri=ANSWER_A, **overrides):
    record = {
        ...
        "redirect_resolution_status": "not_attempted",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A v3.3 record is complete for v3.3 but is not a v3.5 record.
        (
            "v3_3_record",
            _complete_normal_record(schema_version="category_selector_v3.3"),
            False,
        ),
        # A v3.4 record (the frozen v4 selector's own schema) is complete for
        # v3.4 but is not a v3.5 record: the two schemas can disagree about
        # whether the same class was HARD-rejected (R1 vs. v4's detect_leak()),
        # so silently resuming past a v3.4 record would hide exactly the
        # change v5 exists to make.
        (
            "v3_4_record",
            _complete_normal_record(schema_version="category_selector_v3.4"),
            False,
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "schema_version",
        # v3.4: provenance is part of a complete record.
        "redirect_used", "redirect_attempted", "redirect_target",
        "redirect_resolution_status", "redirect_rejected_reason",
        # v3.5: selection provenance is part of a complete record too.
...
def test_the_frozen_selector_files_are_untouched():
    ...
    assert ce.SCHEMA_VERSION == "category_selector_v3.5", "the selector under test is v3.5"


# ---------------------------------------------------------------------------
# 14) v3.5 — redirect provenance reaches JSONL, summary.csv and the manifest
# ---------------------------------------------------------------------------
...
RAW_APOSTROPHE = RESOURCE + "Shin'ichirō_Tomonaga"
OLD_TOMONAGA = RESOURCE + "Sin-Itiro_Tomonaga"
PLAIN_ORIGINAL = RESOURCE + "Plain_Redirect_Source"
PLAIN_TARGET = RESOURCE + "Plain_Redirect_Target"
BAD_SOURCE = RESOURCE + "Bad_Redirect_Source"
BAD_TARGET = RESOURCE + "Bad Redirect Target"

CAT_ALPHA = CATEGORY + "Quantum_electrodynamics"
CAT_BETA = CATEGORY + "Nobel_laureates_in_Physics"


class ProvenanceClient:
    """Models four redirect outcomes in one deterministic fake batch."""
    ...
    def __init__(self):
        ...
        self.queries = []
        self.resources = {
            ANSWER_A: {"label": "Shinya Yamanaka", "categories": (CAT_ALPHA, CAT_BETA)},
            PLAIN_ORIGINAL: {"label": None, "categories": ()},
            PLAIN_TARGET: {"label": "Plain Target", "categories": (CAT_ALPHA,)},
            OLD_TOMONAGA: {"label": None, "categories": ()},
            RAW_APOSTROPHE: {
                "label": "Shin'ichirō Tomonaga",
                "categories": (CAT_ALPHA, CAT_BETA),
            },
            BAD_SOURCE: {"label": None, "categories": ()},
        }
        self.counts = {CAT_ALPHA: 40, CAT_BETA: 120}
        self.redirects = {
            PLAIN_ORIGINAL: PLAIN_TARGET,
            OLD_TOMONAGA: RAW_APOSTROPHE,
    ...
    def _rows(self, qid, query):
        ...
        if qid == "answer_info":
            ...
            if entry is None:
                return []
            return [{"label": self._cell(entry["label"])}] if entry["label"] else [{}]
        if qid == "answer_categories":
            entry = self.resources.get(subject)
            return [] if entry is None else [
                {"cat": self._cell(c)} for c in entry["categories"]
            ]
        if qid == "category_counts":
            return [
                {"cat": self._cell(c), "c": self._cell(n)}
...
PROVENANCE_URIS = [ANSWER_A, PLAIN_ORIGINAL, OLD_TOMONAGA, BAD_SOURCE]


def run_provenance_batch(tmp_path, **kwargs):
    client = ProvenanceClient()
    runner = ce.SparqlRunner(client=client)
    outcome = runner_mod.run_batch(
        answer_uris=PROVENANCE_URIS,
        output_dir=tmp_path,
        runner=runner,
        resolve_redirects=True,
        selector_options={"mode": "recommended", "top_n": 5, "language": "en"},
    ...
    return outcome, client, runner


def test_manifest_reports_v34_selector_and_v11_runner(tmp_path):
    outcome, _, _ = run_provenance_batch(tmp_path)
    manifest = json.loads((tmp_path / "run_manifest.json").read_text(encoding="utf-8"))
    assert manifest["selector_schema_version"] == "category_selector_v3.5"
    assert manifest["runner_schema_version"] == "category_runner_v1.2"
    assert outcome.exit_code == 0


def test_every_jsonl_record_carries_the_four_provenance_fields(tmp_path):
    outcome, _, _ = run_provenance_batch(tmp_path)
    records = read_jsonl(tmp_path / "results.jsonl")
    assert len(records) == len(PROVENANCE_URIS)
    for record in records:
        for field in PROVENANCE_FIELDS:
            assert field in record, f"{record['original_uri']}: missing {field}"


def test_summary_csv_exposes_the_four_provenance_columns(tmp_path):
    run_provenance_batch(tmp_path)
    rows = read_csv_rows(tmp_path / "summary.csv")
    for field in PROVENANCE_FIELDS:
        assert field in runner_mod.SUMMARY_COLUMNS
        assert field in rows[0]
    assert "redirect_used" in rows[0], "the v3 column is preserved"


def test_each_redirect_outcome_is_reported_distinctly(tmp_path):
    _, client, _ = run_provenance_batch(tmp_path)
    by_uri = {r["original_uri"]: r for r in read_jsonl(tmp_path / "results.jsonl")}

    ordinary = by_uri[ANSWER_A]
    assert ordinary["redirect_attempted"] is False
    assert ordinary["redirect_resolution_status"] == "not_attempted"
    assert ordinary["redirect_target"] is None
    assert ordinary["redirect_used"] is False

    plain = by_uri[PLAIN_ORIGINAL]
    assert plain["redirect_attempted"] is True
    assert plain["redirect_resolution_status"] == "followed"
    assert plain["redirect_target"] == PLAIN_TARGET
    assert plain["query_uri"] == PLAIN_TARGET
    assert plain["redirect_used"] is True
    assert plain["redirect_rejected_reason"] is None

    tomonaga = by_uri[OLD_TOMONAGA]
    assert tomonaga["redirect_attempted"] is True
    assert tomonaga["redirect_resolution_status"] == "followed"
    assert tomonaga["redirect_target"] == RAW_APOSTROPHE
    assert tomonaga["query_uri"] == RAW_APOSTROPHE
    assert tomonaga["original_uri"] == OLD_TOMONAGA, "never rewritten"
    assert tomonaga["redirect_used"] is True
    assert tomonaga["selection_status"] == "ok"
    assert "%27" not in tomonaga["query_uri"]

    bad = by_uri[BAD_SOURCE]
    assert bad["redirect_attempted"] is True
    assert bad["redirect_resolution_status"] == "rejected"
    assert bad["redirect_target"] == BAD_TARGET
    assert bad["redirect_rejected_reason"] == ce.REDIRECT_REASON_UNSAFE_TARGET
    assert bad["redirect_used"] is False
    assert bad["query_uri"] == BAD_SOURCE
    assert not [q for q in client.queries if f"<{BAD_TARGET}>" in q], (
        "a rejected target must never be queried"
    )


def test_a_zero_result_lookup_reports_no_redirect(tmp_path):
    class NoRedirect(ProvenanceClient):
        def __init__(self):
            super().__init__()
            self.redirects = {}
    ...
    runner = ce.SparqlRunner(client=client)
    runner_mod.run_batch(
        answer_uris=[PLAIN_ORIGINAL],
        output_dir=tmp_path,
        runner=runner,
        resolve_redirects=True,
    )
    record = read_jsonl(tmp_path / "results.jsonl")[0]
...
def test_a_failed_redirect_query_is_reported_as_query_failed(tmp_path):
    class FailingRedirect(ProvenanceClient):
        def run(self, query):
            if ce.query_id(query) == "redirect":
                self.calls.append("redirect")
                self.queries.append(query)
                return ce.QueryResult(
                    status=ce.QueryStatus.FAILED,
                    error="simulated endpoint failure for redirect",
                    endpoint=self.endpoint,
                    language=self.language,
                )
            return super().run(query)

    client = FailingRedirect()
    runner = ce.SparqlRunner(client=client)
    runner_mod.run_batch(
        answer_uris=[PLAIN_ORIGINAL],
        output_dir=tmp_path,
...
def test_shared_runner_total_still_includes_redirect_queries(tmp_path):
    outcome, client, runner = run_provenance_batch(tmp_path)
    records = read_jsonl(tmp_path / "results.jsonl")
    per_answer = sum(r["sparql_query_count"] for r in records)
    assert per_answer == runner.stats()["queries"]
    assert runner.stats()["client_calls"] == len(client.calls)
    assert client.calls.count("redirect") == 3, "three Answers triggered a lookup"


def test_resume_accepts_a_complete_v35_record(tmp_path):
    run_provenance_batch(tmp_path)
    first = read_jsonl(tmp_path / "results.jsonl")
    preserved = runner_mod.load_previous_records(tmp_path / "results.jsonl")
    assert set(preserved) == {r["original_uri"] for r in first}


def test_resume_rejects_a_v33_record(tmp_path):
    tmp_path.mkdir(parents=True, exist_ok=True)
    record = _complete_normal_record(schema_version="category_selector_v3.3")
    (tmp_path / "results.jsonl").write_text(json.dumps(record) + "\n", encoding="utf-8")
    assert runner_mod.load_previous_records(tmp_path / "results.jsonl") == {}


def test_resume_rejects_a_v34_record(tmp_path):
    """A v4-selector record must never be silently carried forward as a
    finished v5 Answer: the two schemas can disagree about HARD leakage."""
    tmp_path.mkdir(parents=True, exist_ok=True)
    record = _complete_normal_record(schema_version="category_selector_v3.4")
    (tmp_path / "results.jsonl").write_text(json.dumps(record) + "\n", encoding="utf-8")
    assert runner_mod.load_previous_records(tmp_path / "results.jsonl") == {}


@pytest.mark.parametrize("missing", PROVENANCE_FIELDS)
def test_resume_rejects_a_v34_record_missing_provenance(tmp_path, missing):
    tmp_path.mkdir(parents=True, exist_ok=True)
    record = _complete_normal_record()
    record.pop(missing)
    (tmp_path / "results.jsonl").write_text(json.dumps(record) + "\n", encoding="utf-8")
...
def test_two_equivalent_provenance_runs_are_byte_identical(tmp_path):
    def go(directory):
        client = ProvenanceClient()
        runner = ce.SparqlRunner(client=client)
        runner_mod.run_batch(
    ...
    assert go(tmp_path / "a") == go(tmp_path / "b")


def test_input_order_is_preserved_across_mixed_redirect_outcomes(tmp_path):
    run_provenance_batch(tmp_path)
    rows = read_csv_rows(tmp_path / "summary.csv")
    assert [r["original_uri"] for r in rows] == PROVENANCE_URIS
    assert [int(r["input_index"]) for r in rows] == [0, 1, 2, 3]
